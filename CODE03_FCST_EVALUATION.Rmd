...
# get actual values
sFile4Actl <- 'C:/Users/BBB/ARBEIT/US-CCM-DATA-ACTL/RESU/CCM/UNI-AR0_PRB/SV-OFF/FcstSummary_MODEL-UNI-AR0_PRB_SV-OFF_FO1.csv'
fcst_summ <- readCSV( sFile4Actl );  
tsActl = ts( fcst_summ[, "tsyActl"], start = fcstBeg, frequency = 4 )
...
#collect predictive draws for a selected forecast origin for all models
sFO = "FO4"

list_mdl_aux = lapply( list_mdl, function( mdl ) as.matrix( mdl[[sFO]], nrow = dim( mdl$FO4 )[1] ) )

str( list_mdl_aux )

#loop over dates

fcstTime_aux = time( ts( NA, start = fcstBeg, end = fcstEnd, frequency = 4 ) )
fcstYear     = floor( fcstTime_aux )
fcstPeriod   = ( fcstTime_aux - fcstYear + 0.25 ) * 4
fcstTime     = t( apply( cbind( fcstYear, fcstPeriod ), 1, as.vector ) )

# collect density combinations ( average ) over the whole fcst sample
listBigCombDensAve = apply( fcstTime, 1, function(t){ # 
  
  t = c( 2009, 3 )
    
    # loop over models for a given t and given fcst origin
    mat_pd = sapply( list_mdl_aux, function( mat ){  # m_mat_mdl_fo4 = list_mdl_fo4[[m]] 
      ...
      t_mdl = as.vector( mat[ t_indx, -c( 1,2 ) ] )
      t_mdl
    ...
    dens_comb_ave = apply(mat_dens_y, 1, mean)
    
    # get the log score
    tsActl_t = window( tsActl, start = t, end = t ); tsActl_t
    
    # find closest knots 
    x = list_dens[[1]]$x
    
    x_left = x; x_left[ which( x - c(tsActl_t ) >  0 ) ] = 0
    x_rght = x; x_rght[ which( x - c(tsActl_t ) <= 0 ) ] = 1e6
    
    x_left_t = max( x_left )
    x_left_t_indx = which.max( x_left )
    
    x_rght_t = min( x_rght )
    
    y_left_t = dens_comb_ave[ x_left_t_indx     ]
    y_rght_t = dens_comb_ave[ x_left_t_indx + 1 ]
    y_t      = log( approx( c( x_left_t, x_rght_t ), c(y_left_t, y_rght_t), xout = tsActl_t, method = "linear")$y )
    
    cbind( x_left, x_rght )
    
    
    # draw individual densities
    plot( NA, ylim = dens_range, xlim = comb_range, main = paste0("SIM density combination (AVG): ", t[1],"Q", t[2], " ",sFO ) )
    lapply( list_dens, function(d) lines( d$x, d$y ) )

    # draw density combination ( average )
    lines( x, dens_comb_ave, col = 2, lwd = 2 )
    abline( v = tsActl_t, lwd = 2, col = 5 )
    
    list( x = list_dens[[1]]$x, y = dens_comb_ave, logScore = y_t, tsActl = tsActl_t )

})

i = 107

plot( NA, xlim = range( listBigCombDensAve[[i]]$x ), ylim = range( listBigCombDensAve[[i]]$y ), ylab ="" )
lines( listBigCombDensAve[[i]]$x, listBigCombDensAve[[i]]$y, lty = 1, lwd = 2 )
